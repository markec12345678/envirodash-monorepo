...
import { Flame, RefreshCw, X, Loader2, AlertTriangle, CheckCircle2 } from 'lucide-react'
import { fetchWildfire, type WildfireQuery } from './api'
import { STATUS_COLORS, STATUS_LABELS_EN, type MonitorLocation } from '@envirodash/core'

export interface WildfireMonitorProps {
  query?: WildfireQuery
  onClose?: () => void
  onRefresh?: () => void
  showClose?: boolean
...
export function WildfireMonitor({
  ...
  const load = async () => {
    ...
    setError(null)
    try {
      const data = await fetchWildfire(query)
  ...
  }, [query.lat, query.lng, query.area, query.country])

  const active = locations.find((l) => l.id === activeId)

  return (
    <div className="fixed right-4 top-16 z-[60] w-[380px] max-h-[85vh] overflow-hidden rounded-xl border-2 border-orange-500/30 bg-white shadow-2xl dark:bg-zinc-900">
      <div className="bg-gradient-to-r from-orange-500 to-red-600 p-4 text-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Flame className="h-5 w-5" />
            <div>
              <h2 className="text-lg font-bold">🔥 Wildfire Risk</h2>
              <p className="text-xs text-orange-100">Fire Weather Index</p>
            </div>
          </div>
      ...
      <div className="max-h-[calc(85vh-100px)] overflow-y-auto p-4">
        ...
        {loading && locations.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-zinc-500">
            ...
            <p className="text-sm">Loading fire risk data…</p>
          </div>
        )}
                ...
                <li key={loc.id}>
                  ...
                  >
                    ...
                    <span className="flex items-center gap-2">
                      ...
                      {loc.value != null && <span className="font-bold">{loc.value}</span>}
                    </span>
                  </button>
                </li>
              ))}
              ...
              <div className="rounded-lg border bg-white p-3 dark:bg-zinc-800">
                ...
                {active.metrics && (
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <Metric label="FWI" value={active.metrics.fwi} unit="/ 100" highlight />
                    <Metric label="Temp" value={active.metrics.temperature} unit="°C" />
                    <Metric label="Humidity" value={active.metrics.humidity} unit="%" />
                    <Metric label="Wind" value={active.metrics.wind} unit="km/h" />
                    <Metric label="Precip" value={active.metrics.precipitation} unit="mm" />
                  </div>
                )}
                {active.description && (
        ...
        {!loading && !error && locations.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-zinc-500">
            ...
            <p className="text-sm">No fire risk data available.</p>
          </div>
        )}
      </div>
    </div>
