...
import { Droplets, RefreshCw, X, Loader2, AlertTriangle, CheckCircle2 } from 'lucide-react'
import { fetchFloods, type FloodQuery } from './api'
...
export function FloodMonitor({
  ...
  const load = async () => {
    ...
    setError(null)
    try {
      const data = await fetchFloods(query)
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-sky-100">Open-Meteo · Real-time</p>
            </div>
          </div>
      ...
      <div className="max-h-[calc(85vh-100px)] overflow-y-auto p-4">
        ...
        {loading && locations.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-zinc-500">
            ...
            <p className="text-sm">Loading flood risk data…</p>
          </div>
        )}
              ...
              <div className="rounded-lg border bg-white p-3 dark:bg-zinc-800">
                ...
                {active.metrics && (
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <Metric label="Flood Risk" value={active.metrics.flood_risk} unit="/ 100" highlight />
                    <Metric label="Precip now" value={active.metrics.precipitation_now} unit="mm/h" />
        ...
        {!loading && !error && locations.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-zinc-500">
            ...
            <p className="text-sm">No flood risk data available.</p>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div
      className={`rounded border px-2 py-1 ${
        highlight ? 'border-sky-300 bg-sky-50 dark:bg-sky-950/30' : 'bg-zinc-50 dark:bg-zinc-900'
      }`}
    >
