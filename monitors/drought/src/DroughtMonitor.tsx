...
import { Sun, RefreshCw, X, Loader2, AlertTriangle, CheckCircle2 } from 'lucide-react'
import { fetchDroughts, type DroughtQuery } from './api'
...
export function DroughtMonitor({
  ...
  const load = async () => {
    ...
    setError(null)
    try {
      const data = await fetchDroughts(query)
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-amber-100">Open-Meteo · Real-time</p>
            </div>
          </div>
      ...
      <div className="max-h-[calc(85vh-100px)] overflow-y-auto p-4">
          ...
          <>
            ...
            {active && (
              <div className="rounded-lg border bg-white p-3 dark:bg-zinc-800">
                ...
                {active.description && (
                  <p className="mb-2 text-xs text-zinc-500">{active.description}</p>
                )}
                {active.metrics && (
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <Metric label="Drought Score" value={active.metrics.drought_score} unit="/ 100" highlight />
                    <Metric label="Temperature" value={active.metrics.temperature} unit="°C" />
                    <Metric label="Humidity" value={active.metrics.humidity} unit="%" />
                    <Metric label="Precip 24h" value={active.metrics.precipitation_24h} unit="mm" />
                    <Metric label="Soil moist (top)" value={active.metrics.soil_moisture_top} unit="m³/m³" />
                    <Metric label="Soil moist (deep)" value={active.metrics.soil_moisture_deep} unit="m³/m³" />
                  </div>
                )}
                {active.lastUpdated && (
                  <p className="mt-2 text-[10px] text-zinc-400">Updated: {new Date(active.lastUpdated).toLocaleString()}</p>
                )}
              </div>
            )}
        ...
        {!loading && !error && locations.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-zinc-500">
            ...
            <p className="text-sm">No drought data available.</p>
          </div>
        )}
      </div>
    </div>
...
function Metric({
  ...
  const display = typeof value === 'number' ? (Number.isFinite(value) ? (value < 1 ? value.toFixed(3) : value < 10 ? value.toFixed(2) : value.toFixed(1)) : '—') : value ?? '—'
  return (
    <div
      className={`rounded border px-2 py-1 ${
        highlight ? 'border-amber-300 bg-amber-50 dark:bg-amber-950/30' : 'bg-zinc-50 dark:bg-zinc-900'
      }`}
    >
